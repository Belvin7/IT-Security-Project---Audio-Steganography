...
class FrameHeader:
    ...
    def init_header_params(self, buffer):
        ...
        :param buffer: buffer that contains the mp3 file bytes, starts with the first bytes of the header.
        """
    ...
    def __set_crc(self):
        ...
        """
        self.__crc = bool(self.__buffer[1] & 0x01)
    ...
    def __set_mode_extension(self):
        ...
        """
        if self.__layer == 3:
            value = ((self.__buffer[3] & 0x30) >> 4) % MAX_BYTE_VALUE
            self.__mode_extension = ModeExtension(value)
    ...
    def __set_private(self):
        ...
        If set, the frame size is 1 byte larger.
        """
        self.__private = bool(self.__buffer[2] & 0x01)
    def __set_copyright(self):
        """
        If set, the frame size is 1 byte larger.
        """
        self.__copyright = bool(self.__buffer[3] & 0x08)
    def __set_original(self):
        """
        If set, the frame size is 1 byte larger.
        """
        self.__original = bool(self.__buffer[3] & 0x04)
    ...
    @property
    def bitstring(self):
        return " ".join([mp3utils.byteToBits(byte) for byte in self.__buffer[0:4]])
